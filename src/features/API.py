...
class GoBoardUI:
    def __init__(self, master, board_logic):
        self.master = master
        self.board_logic = board_logic # Your Board() instance
        self.board_size = board_logic.size[0] # Assuming square 19x19
        ...
        # Setup Canvas
        self.canvas = tk.Canvas(master, width=canvas_width, height=canvas_height, bg=self.wood_color)
        self.canvas.pack()
        ...
        # 1. Track where we are in history (Start at the end)
        self.view_index = len(self.board_logic.game.moves)

        # 2. Allow the canvas to accept keyboard inputs
        self.canvas.focus_set() 

        # 3. Bind Arrow Keys
        self.canvas.bind("<Left>", self.go_back)
        self.canvas.bind("<Right>", self.go_forward)
        ...
        self.clear_btn.place(x=50, y=20) # Adjust position as needed
        ...
        self.invert_btn.place(x=200, y=60)

        #Analyze Button
        self.analyze_btn = tk.Button(self.master, text="Launch Analysis", command=lambda: self.analyze_area(self.selected_area))
        self.analyze_btn.place(x=200, y=100) # Adjust x/y as needed
    ...
    def update_board_to_current_index(self):
        ...
        """
        # 1. Get the list of moves we want to show (the "slice" of history)
        # Note: We rely on self.board_logic.game.moves being the Master List
        moves_to_replay = self.board_logic.game.moves[:self.view_index]
        
        # 2. Reset the logic board to this state
        # Your Board class already has a method for this!
        try:
    ...
    def analyze_area(self, positions):
        """
        Placeholder for area analysis logic.
        You can integrate your analysis feature here.
        """
        ...
        analize = Analizer("/Users/marcelomiranda/Documents/IMT-L2/Commande_entreprise/games/test1.sgf")

        analize.deep_turn_analysis(turn = self.view_index - 1, selection=self.selected_area, invert_selection=self.invert_selection)
        print(analize.turn_analysis[self.view_index - 1])
